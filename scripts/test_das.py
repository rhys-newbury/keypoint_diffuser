...
# Prediction args
arg_parser.add_argument('-a', '--annotation-json', type=str, default='annotations/cap.json',
...
arg_parser.add_argument('-m', '--checkpoint-path', type=str, default='trained_merger.pt',
...
def run_prediction(opt):
    ...
    for i in tqdm.tqdm(range(0, len(kpn_ds), opt.batch_size), unit_scale=opt.batch_size):
        ...
        with torch.no_grad():
            ...
            batch = collate_fn(T_nb)  # assumes collate_fn knows how to stack dictionaries correctly
            ...
            for idx in range(key_points.shape[0]):
                ...
                # viz_pointcloud_and_keypoints(Q[idx], kp_pcn, title='Keypoints in PCN space', idx=f"pcn_{idx}")
                # viz_pointcloud_and_keypoints(Q_nb[idx][:, :3], kp_unitbox, title='Keypoints in Unit Box space', idx=f"ub_{idx}")
...
if __name__ == '__main__':
    ...
    print(f"mIoU at threshold 0.1: ", mIoU_curve_plot(kpn_ds, predicted, opt.pcd_path) * 100)
    # e.g.,
