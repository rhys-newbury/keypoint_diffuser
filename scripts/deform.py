...
def apply_general_deformation(point_cloud: torch.Tensor, 
    ...
    """
    Applies a range of deformations (stretch, bend, twist, taper, noise) using a transformation matrix.
    ...
    stretch_factors = 1.0 + (torch.rand(batch_size, device=device) * (max_stretch_factor - 1.0))
    ...
    noise_std_factors = torch.rand(batch_size, device=device) * max_noise_std if apply_noise else None
    ...
    if apply_bend:
        bending_offsets = bending_factors.view(batch_size, 1) * torch.sin(point_cloud[:, :, 0] * 3.14)
        deformation_matrix[:, 1, 0] = bending_offsets.mean(dim=1)  # Apply bending in Y direction

    # 3. Twisting (Rotation about X-axis)
    if apply_twist:
        theta = twist_factors.view(batch_size, 1) * point_cloud[:, :, 0].mean(dim=1, keepdim=True)  # Avg X for rotation
    ...
    deformed_cloud = torch.bmm(point_cloud, deformation_matrix.transpose(1, 2))  # Apply transformation

    # 5. Adding Noise (After transformation)
    if apply_noise:
        noise = torch.randn_like(deformed_cloud) * noise_std_factors.view(batch_size, 1, 1)
