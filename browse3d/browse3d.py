...
parser.add_argument(
    ...
    "--visuals",
    type=str,
    default=os.path.join("browse3d", "configs", "default.json"),
    help="",
)
...
@route("/rel/<path:path>")
def main(**args):
    with open(opt.visuals) as f:
        visuals = json.load(f)

    log_dir_dym = os.path.join(log_dir, args["path"]) if "path" in args else log_dir
    ...
    return template(
        "browse3d", title=opt.log_dir, samples=samples, opt=opt, visuals=visuals
    )
