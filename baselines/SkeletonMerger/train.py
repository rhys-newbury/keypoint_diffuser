...
arg_parser.add_argument('-m', '--checkpoint-path', '--model-path', type=str, default='trained_merger.pt',
...
def feed(net, optimizer, loader, train, shuffle, batch, epoch):
    running_loss = 0.0
    running_lrc = 0.0
    running_ldiv = 0.0
    net.train(train)
    ...
    with contextlib.suppress() if train else torch.no_grad():
        ...
        loop = tqdm(enumerate(loader), total=len(loader), desc='Train' if train else 'Eval')
        ...
        for i, batch_x in loop:
            ...
            loop.set_postfix({
                ...
                'Lrc': f'{running_lrc / (i + 1):.4f}',
                'Ldiv': f'{running_ldiv / (i + 1):.4f}',
                'epoch': epoch,
...
if __name__ == '__main__':
    ...
    dataset = H5Dataset(h5_files, normalize=True, include_label=False,subclasses=(ns.subclass,), )
    ...
    dataset_test = H5Dataset(h5_files_test, normalize=True, include_label=False,subclasses=(ns.subclass,), )
    loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=batch, shuffle=True, num_workers=0)
    ...
    for epoch in range(ns.epochs):
        feed(net, optimizer, loader, True, True, batch, epoch)
        feed(net, optimizer, loader_test, False, False, batch, epoch)
        torch.save({
            'epoch': epoch,
            'model_state_dict': net.state_dict(),
        }, f"{epoch}_{ns.checkpoint_path}")
