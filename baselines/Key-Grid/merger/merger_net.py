...
def gen_grid2d(grid_size: int, left_end: float=-1, right_end: float=1) -> torch.Tensor:
    ...
    # coords = torch.linspace(left_end, right_end, 16)

    # x, y, z = torch.meshgrid(coords, coords, coords, indexing='ij')  # Ensure consistent axis ordering
...
class Net(nn.Module): 
    ...
    def draw_lines(self,normal_point,paired_joints):
        ...
        squared_dist = (before_start + after_end + between_start_end).reshape(bs, n_points,N)
