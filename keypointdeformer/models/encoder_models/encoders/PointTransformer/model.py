...
class Point(Dict):
    ...
    - "serialized_depth": depth of serialization, 2 ** depth * grid_size
        describe the maximum of point cloud range;
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "batch" not in self.keys() and "offset" in self:
            self["batch"] = offset2batch(self.offset)
        elif "offset" not in self.keys() and "batch" in self:
...
class PDNorm(PointModule):
    ...
    def forward(self, point):
        ...
        condition = (
            point.condition if isinstance(point.condition, str) else point.condition[0]
        )
        if self.decouple:
...
class SerializedUnpooling(PointModule):
    ...
    def forward(self, point):
        assert "pooling_parent" in point
