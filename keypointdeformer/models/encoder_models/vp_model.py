...
class VPPrecond(torch.nn.Module):
    ...
    def edm_sampler(
        self, code, class_labels=None, randn_like=torch.randn_like,
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
            ...
            denoised = self(x_hat, t_hat.reshape(1).repeat(16), context=code).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < num_steps - 1:
                denoised = self(x_next, t_next.reshape(1).repeat(16), context=code).to(torch.float64)
