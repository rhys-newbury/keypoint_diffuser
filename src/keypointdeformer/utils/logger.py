...
def info(message, *lines, ostream=sys.stdout, bold=False):
    color = ansi.WHITE_B if bold else ansi.WHITE
    if message:
        string = (
            "{}{}: " + message + ("{}\n" if lines else "{}") + "\n".join(lines) + "{}"
        )
        ...
        print(
            string.format(
                ansi.WHITE, time.strftime("%Y-%m-%d %H-%M-%S", time.gmtime()), ansi.ENDC
            ),
            file=ostream,
        )
...
def success(message, *lines, ostream=sys.stdout):
    if message:
        ...
        print(
            string.format(
                ansi.BLUE, time.strftime("%Y-%m-%d %H-%M-%S", time.gmtime()), ansi.ENDC
            ),
            file=ostream,
        )
