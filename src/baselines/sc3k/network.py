...
"""
 ...
 The last layer of and Pointnetfeat has modified to get [N, 512] features
"""
...
class Unsupervised_kpnet_without_Residual_block(nn.Module):
    ...
    Sub-modules;
    1. pointnet features extractor for point clouds
    2. MLP and Softmax
    """
...
class Unsupervised_kpnet(nn.Module):
    ...
    Sub-modules;
    1. Pointnet features extractor for point clouds
    2. Residual block, Conv1D and Softmax
    """
    ...
    def forward(self, pc):
        ...
        x = self.conv23(x)  # [B x cfg.key_point x 2048]
        x = self.softmax(
            x
        )  # [B x cfg.key_point x 2048] => [B x cfg.key_point x 2048{0 to 1}]
        x = torch.bmm(
            x, pc
        )  # [Bx cfg.key_point x2048] <-> [Bx2048x3]  =>  [Bx cfg.key_point x3]
