...
class XConv(torch.nn.Module):
    ...
    def forward(self, x: Tensor | None, pos: Tensor):
        ...
        knn_idx = torch.topk(pw_dist, self.kernel_size * self.dilation, largest=False)[
            1
        ]  # (B, N, K * dilation)
        ...
        x_star = self.mlp1(d.reshape(B * N * self.kernel_size, -1)).reshape(
            B, N, self.kernel_size, -1
        )  # (B, N, K, C_delta)
        if x is not None:
            x = x[:, None, :, :].repeat(1, N, 1, 1)  # (B, N, 1, F_in)
            x = torch.gather(
                x, 2, knn_idx[:, :, :, None].expand(-1, -1, -1, self.in_channels)
            )
        ...
        transform_matrix = self.mlp2(
            d.reshape(B * N, self.kernel_size * D)
        )  # (B * N, K, K)

        x_transformed = torch.matmul(
            x_star, transform_matrix
        )  # (B * N, C_delta + C_in, K)
