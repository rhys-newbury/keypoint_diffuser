...
class Head(nn.Module):  # Decoder unit, one per line segment
    ...
    def forward(self, KPA, KPB):
        dist = torch.mean(
            torch.sqrt(1e-3 + (torch.sum(torch.square(KPA - KPB), dim=-1)))
        )
        count = min(200, max(15, int((dist / 0.01).item())))
        device = dist.device
        self.f_interp = (
            torch.linspace(0.0, 1.0, count).unsqueeze(0).unsqueeze(-1).to(device)
        )
...
class Net(nn.Module):
    ...
    def draw_lines(self, normal_point, paired_joints):
        ...
        t = (diff_to_start @ paired_diff.unsqueeze(-1)).squeeze(-1) / (
            1e-8 + paired_diff.square().sum(dim=-1, keepdim=True)
        )
        ...
        between_start_end = (
            ...
            * (
                grid
                - (start.unsqueeze(-2) + t.unsqueeze(-1) * paired_diff.unsqueeze(-2))
            )
            .square()
            .sum(dim=-1)
        )

        N = normal_point.shape[1]

        squared_dist = (before_start + after_end + between_start_end).reshape(
            bs, n_points, N
        )
    ...
    def get_keypoints(self, input_x):
        ...
        kp_heatmaps = (kp_heatmaps == kp_heatmaps.max(dim=2, keepdim=True)[0]).to(
            input_x
        )
