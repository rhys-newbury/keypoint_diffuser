...
x = np.load("viz_target_sampled_points.npy")
deformed = np.load("viz_target_deformed_points.npy")
decoded = np.load("viz_decoded.npy")
...
pcd = np_to_pcd(x[i, ...])
pcd2 = np_to_pcd(deformed[i, ...])

cd.append(chamfer_distance_numpy(x[i, ...][..., :3], deformed[i, ...]))

print(max(cd), cd.index(max(cd)))

R = pcd2.get_rotation_matrix_from_xyz((np.pi / 2, 0, 0))  # 90° around X
pcd2.rotate(R, center=(0, 0, 0))

kpts = o3d.geometry.PointCloud()
kpts.points = o3d.utility.Vector3dVector(kp_deformed[i])
...
convert_green_to_transparent_opencv(
    "plane_view.png", "plane_view_deformed_transparent_kp.png"
)
